...
import type { Volume } from "@/src/apidata/Volumes";

type VolumeIssuesProps = {
  volumeData: Volume[];
...
export default function VolumeIssues({
  ...
  const paginatedData = useMemo(() => {
    ...
    return volumeData.slice(start, start + pageSize);
  }, [volumeData, currentPage, pageSize]);

  const startIndex = totalItems === 0 ? 0 : (currentPage - 1) * pageSize + 1;
  const endIndex = Math.min(currentPage * pageSize, totalItems);

  const handleVolumeClick = (volumeId: number) => {
    router.push(`/volumes/${volumeId}`);
          ...
          <table className="w-full min-w-[640px] border-collapse text-left">
            ...
            <tbody>
              {paginatedData.length === 0 ? (
                ...
                paginatedData.map((volume, index) => (
                  <tr
                    key={volume.id}
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      <button
                        ...
                        className="text-left text-sm font-semibold text-[#092151] transition hover:text-[#036eb6] cursor-pointer"
                      >
                        {volume.title}
                      </button>
                    </td>
                    <td className="px-5 py-4 text-right">
                      <button
                        type="button"
                        onClick={() => handleVolumeClick(volume.id)}
                        className="inline-flex items-center gap-1.5 rounded-lg border border-blue-200 bg-blue-50 px-3 py-1.5 text-xs font-semibold text-[#024081] transition hover:bg-blue-100 cursor-pointer"
                      >
                        View issues
